...
## File Overview ###
### finetuned.pt
USE THIS MODEL. (unless you have good reason not to). This was finetuned on our own prop images of mannequins laying in brown dirt (as close as possible to irl environment in OK). Ensured that this model recognizes outlines of relatively small objects of interest regardless of color contrast. Trigger-happy with labeling tents as humans but that's preferable to the alternative.
### best.pt
DO NOT USE in prod. (unless you have good reason to). This is best model BEFORE FINETUNING; can possibly use this as base for future fine-tuning and fine-tuning experiments. 
...
### slice_dataset.py
Takes folder of training images and labels, splits them into smaller image chunks for SAHI, and gives a new folder of datasets of chunks of the original images.
...
### docs.txt
Just repeat of some stuff that's here. My personal notes.
...
- IMPORTANT: you shouldn't need to do a whole new training run; just work off either best.pt or finetuned.pt; this already has the brain to recognize different types of people and tents, just need to fine-tune off whatever specific things we expect at comp from there.
- Furthermore, I'd actually recommend using Kaggle for fine-tuning unless there's seriously heavy workloads that need to be done. Can ask chat for help there, it's pretty simple to run stuff in Kaggle (although takes a while). See my Kaggle workbook [here](https://www.kaggle.com/code/professorfrink/finetuning).

## Note on Kaggle
Kaggle stops you at 12 hours. Because of this, my earlier scripts when I'd still use Kaggle would always have a timer that'd stop training at like 11.5 hours to be safe and securely store the results in persistent memory (make sure when you run the Kaggle script you run with 'Save version' button in top-right). My [Kaggle training notebook](https://www.kaggle.com/code/professorfrink/notebookb441752b91).
